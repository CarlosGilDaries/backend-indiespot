async function editContentForm() {
  ...
  async function loadContentData(id) {
    try {
      ...
      toggleContentTypeFields(content.type);
    } catch (error) {
  ...
  function toggleContentTypeFields(type) {
    const shouldChange = document.getElementById('change-content-file').checked;
    if (!shouldChange) return;

    const contentType = document.getElementById('type-content');
    const singleContent = document.getElementById('single-content');
    const hlsContent = document.getElementById('hls-content');
